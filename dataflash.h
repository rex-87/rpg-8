...
/* taille du pattern (en octets) */
#define PAT_BLENGTH ( (uint16_t)256 )
...
void Buffer_Write(uint8_t BufferNo, uint16_t IntBufAdr, uint16_t No_of_bytes, uint8_t *BufferPtr);
void Buffer_Read(uint8_t BufferNo, uint16_t IntBufAdr, uint16_t No_of_bytes, uint8_t *BufferPtr);

void Page_To_Buffer(uint8_t BufferNo, uint16_t PageAdr);
void Buffer_To_Page(uint8_t BufferNo, uint16_t PageAdr);

void Cont_Flash_Read_Enable (uint16_t PageAdr, uint16_t IntBufAdr);

void DF_write(unsigned long ulAddress, uint8_t ucData);
