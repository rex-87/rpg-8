...
*					IntBufAdr	->	Internal page address
...
*
******************************************************************************/
void Buffer_Read( uint8_t  BufferNo
                , uint16_t IntBufAdr
                ...
                )
{
	...
	if (1 == BufferNo)							//read byte(s) from buffer 1
	{
		...
		DF_SPI_RW((uint8_t)(IntBufAdr>>8));//upper part of internal buffer address
		DF_SPI_RW((uint8_t)(IntBufAdr));	//lower part of internal buffer address
		DF_SPI_RW(0x00);						//don't cares
...
*
******************************************************************************/
void Buffer_Write( uint8_t BufferNo
                 , uint16_t IntBufAdr
                 , uint16_t No_of_bytes
                 , uint8_t *BufferPtr
                 )
...
*
******************************************************************************/
void Page_To_Buffer( uint8_t BufferNo
                   , uint16_t PageAdr
				   )
{
	DF_CS_inactive;												//make sure to toggle CS signal in order
	...
	if (1 == BufferNo)											//transfer flash page to buffer 1
	{
		...
		DF_SPI_RW((uint8_t)(PageAdr >> (16 - PageBits)));	//upper part of page address
		DF_SPI_RW((uint8_t)(PageAdr << (PageBits - 8)));	//lower part of page address
		DF_SPI_RW(0x00);										//don't cares
	}
	else
...
*
******************************************************************************/
void Buffer_To_Page( uint8_t  BufferNo
                   , uint16_t PageAdr
				   )
{
	DF_CS_inactive;												//make sure to toggle CS signal in order
...
static uint16_t lastWAdr = 0;
static uint16_t lastRAdr = 0;
...
void DF_write(uint32_t ulAddress, uint8_t ucData)
{
	...
	bufferAdr = (uint16_t)(ulAddress & 0x00FF);
	wPageAdr  = (uint16_t)((ulAddress >> 8) & 0x07FF);
...
*
******************************************************************************/
uint8_t DF_read(uint32_t ulAddress)
{
	uint16_t bufferAdr;
	uint16_t rPageAdr;
	uint8_t ucData;
	...
	bufferAdr = (uint16_t)(ulAddress & 0x00FF);
	rPageAdr  = (uint16_t)((ulAddress >> 8) & 0x07FF);

	if( (lastRAdr != rPageAdr) )
	{
